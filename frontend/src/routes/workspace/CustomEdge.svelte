<script lang="ts">
	...
	function handleMouseMove(event: MouseEvent) {
		...
		const svgElement = (event.currentTarget as SVGElement).closest('svg') as SVGSVGElement;
		if (!svgElement) return;

		const transformedPoint = transformCoordinates(event, svgElement);
		if (!transformedPoint) return;

		const nearestPoint = getClosestPoint(pathElement, transformedPoint.x, transformedPoint.y);

		// Check if the new point would be too close to existing points
		const tooClose = hoverPoints.some(
			(p) => Math.hypot(p.x - nearestPoint.x, p.y - nearestPoint.y) < CONFIG.MIN_POINT_DISTANCE
		);
...
>
	...
	/>
	<BaseEdge path={edgePath} style={edgeStyle} {interactionWidth} />
	<!-- {markerEnd} -->
</g>
...
{#each hoverPoints as point (point.id)}
	<EdgeLabel x={point.x} y={point.y} transparent class="nodrag nopan">
		<div class="hover-point" transition:fade={{ duration: CONFIG.FADE_DURATION, easing: cubicOut }}>
			<button
				class="delete-button"
