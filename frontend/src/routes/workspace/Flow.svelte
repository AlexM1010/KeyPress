...
<script lang="ts">
	...
	const runButtonStatus = $derived(
		hasRunAlert && !isRunAlertHeld ? { icon: Play, color: 'text-foreground' } : executionStatus
	);
	...
	async function handleRunFlow() {
		try {
			...
			const hasStartNode = currentFlowData.nodes.some((node) => node.type === 'StartNode');
			if (!hasStartNode) {
	 ...
	 */
	function addStatusMessage(msg: { id: string; type: string; message: string; nodeId?: string }) {
	...
	function setupEventListeners() {
		...
		eventUnsubscribers.push(
			Events.On('execution-error', ({ data }) => {
		...
		eventUnsubscribers.push(
			Events.On('execution-nodes-skipped', ({ data }) => {
...
<div class="flow-container flex">
	...
	<LeftPanelToggleButton {isLeftPanelExpanded} {toggleLeftPanel} />
           ...
           the only feedback that a box-drag caught what the user meant it to. -->
			{#if hasMultiSelection}
				<Panel position="top-left">
					<div
						class="nav-button-container flex-center flex-gap"
						role="toolbar"
			...
			<Panel position="top-right">
				<div class="flex flex-col items-end">
                 ...
                 why just below. -->
						...
						/>
						<!-- Run Flow Button -->
						<button class="flow-button" onclick={handleRunFlow} disabled={isExecuting}>
							<RunIcon
                 ...
                 point. -->
						<button class="flow-button" onclick={() => onLayout('LR')}>
	...
	<StatusPanelToggleButton {isStatusPanelExpanded} {hasStatusPanel} {toggleStatusPanel} />

	<!-- Status Panel -->
	<StatusPanel {isStatusPanelExpanded} {statusMessages} {executionStatus} />
</div>
